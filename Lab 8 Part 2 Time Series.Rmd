...
ggplot(data = toolik_ts, aes(x = date, y = mean_airtemp)) +
  geom_line() +
  labs(x = "Date",
       y = "Mean daily air temperature (Celsius)\n at Toolik Station") #the \n is a line break so the title will be separated across two lines rather than one long line 
```
...
toolik_month <- toolik_ts %>% 
  index_by(yr_mo = ~yearmonth(.)) %>% #this is like group by but specific to time series tibbles (can do it with year and month or year and week or year and day)
...
slide(test, ~.x, .before = 2, .after = 2) #this is our moving average
...
ggplot() +
  ...
  geom_line(data = roll_toolik_15, aes(x = date, y = ma_15d), color = "orange") + #this is the y based on the moving average from the 15 day window
  geom_line(data = roll_toolik_61, aes(x = date, y = ma_61d), color = "blue") +
  theme_minimal() # this is the y based on the moving average from the 61 day window
```
...
```{r}
toolik_ts %>%
  ACF(mean_airtemp) %>% #this is the autocorrelation function
  autoplot()
...
toolik_month %>% 
  ...
  autoplot()
```
Day: If we do a lag of one day do we think there would be a high correlation (yes!) because the acf is high, as you get farther and farther away, observations a month later are less and less similar as time goes on

Month Analysis: [add later]
...
toolik_dec <- toolik_month %>% 
  model(STL(monthly_mean_temp ~ season(window = Inf))) #window is wide open with Inf

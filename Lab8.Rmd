...
voles <- read_sf(dsn = here("redtreevoledata"), #in the redtreevoledata folder, looking for this folder and find the spatial data that is defined by ds033
                 layer = "ds033") %>% 
  janitor::clean_names() %>%
...
humboldt <- read_sf(dsn = here("redtreevoledata"), 
  ...
  st_set_crs(4326) %>% #assign coordinate reference system becuase there is not a .prj (make sure this is right because if it is wrong then all transformations from here on out will be wrong)
  st_transform(crs = 32610) #this turns it from one coordinate reference system to another 
...
ggplot() +
  ...
  geom_sf(data = voles, 
          color = "orange", 
          alpha = 0.7, #this makes them slightly transparent
          size = 2) + #this makes them bigger (size of the points)
  theme_minimal()

#when you do ggplot it stacks the layers from the bottom to the top
```
...
```{r}
voles_sp <-as(voles,"Spatial") # converts to object "Spatial"
...
```{r}
voles_density <- density(voles_full, sigma = 5000) #If you were to say sigma = 1 (that is a standard deviation of 1 meter, we are expanding the standard deviation outward)
plot(voles_density)
```
...
crs(vole_raster) <- crs(voles) #assigning crs to match voles dataset
...
gfunction_long <- gfunction %>% 
  as.data.frame() %>% #making it a dataframe
  pivot_longer(cols = obs:hi, names_to = "model", values_to = "g_val") #this is taking all the columns from observed to high, the names are turned into a column called model, then going to take the values and turn them into g_val
 
# Then make a graph in ggplot:
ggplot(data = gfunction_long, aes(x = r, y = g_val, group = model)) + #color the lines by the 
  geom_line(aes(color = model))

#we can say that the blue line is clustered, because as we go a little bit out we see that there is a lot of points clustered, more than if they were just spatially random (sophia note)
```
